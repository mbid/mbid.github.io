...
This means that our type checker accepts a program only if it can assign a uniquely determined, concrete type to every expression and variable in the program.
This means that our type checker rejects some sensible programs, for example this one:
```typescript
function id(x) {
    return x;
}
...
We represent type variables in our Eqlog program as a `Type` elements which cannot be obtained by applying one of the type constructors to other `Type` elements.
...
Our flavor of polymorphism is usually called *prenex* or *rank 1* polymorphism.
...
- Variables introduced using function statements have polytypes:
  ```eqlog
  Axiom
...
- Variables introduced using let statements have monotypes:
  ```eqlog
  Axiom
      ConsStmtListNode(_, head, tail)
      ...
      ;
  ```
  This is similar to Rust's type system, where local variables must have monotypes, but different from TypeScript's type system, where such variables can have polytypes.
  But this is an arbitrary decision, not a fundamental limitation, that lets us reuse most of the `ExprType` machinery we've introduced in the last post without much adaptation.
...
If the polytype appears in the global module scope, then all type variables in it are universally quantified.
But if a polytype appears in the body of function, then only those type variables that are not already introduced by arguments of the function are bound.
...
```
Suppose the type of `x` is `MonoType(a)` and that the type of `y` is `MonoType(b)` for type variables `a` and `b`.
Then the type of `id` is `PolyType((a) => a)`, and since `id` appears in the global module scope, `a` is bound.
The type of `const_x` is `PolyType((b) => a)`, but since it appears in the body of the `id` function, so that `a` is in the ambient type context, only `b` is bound.

Because of local functions such as the one above, we need a way to keep track of which type variables were already introduced in the body of a function, and we need to propagate this information into relevant subnodes.
...
`TypesInContext(tys, ctx)` should hold if and only if every type in `tys` is in `ctx`; we've seen axioms encoding similar properties before, so they're omitted here.
...
A type context which contains all the types of the base type context, but may contain additional types.
...
Datalog cannot support recursively applicable rules containing negations, and Eqlog does not support it at all.
...
To prepare, we close type contexts under type constructors:
```eqlog
...
This is relevant for functions with arguments whose types that involve a type variable but are not directly equal it, for example in this function:
```typescript
function apply (f, x) {
    return f(x);
}
...
Axiom
    ...
    ;
```
Now a type element is unbound if and only if it is in the ambient type context.


## Instantiation

Our function `VarInExpr : Var -> GeneralType` can result into a polytype, whereas we still expect the function `ExprType : ExprNode -> Type` to result into a type in the current context, i.e., into a monotype.
...
```
Each `Instantiation` element represents a (partial) maps `Type -> Type`, and the `Instantiate` function represents the application of such maps to `Type` elements.
...
We associate an `Instantiation` element to every usage of a variable with polytype and equate the expression type with an instantiation of the polytype scheme.
```
...
The typing rules we added in the previous post put constraints on the `ExprType` of polymorphic variables based on their usage, but we still need to encode constraints on it based on the variable's polytype scheme.
...
Unbound type variables cannot be freely instantiated, since they are bound in the ambient type context of the polytype already.
In case of the convoluted identity function above, this means that every instantiation of `const_x : (b) => a` must map `a` onto itself.

Since we're not allowing higher-ranked types, we do not allow return values with polytypes from functions; even if we return a variable with polymorphic type, it is instantiated into a monotype first.
...
Now that we've added support for polymorphism, this check is evidently overly restrictive, but we cannot remove it entirely.
...
This means that also the type of `x` is a undetermined (i.e., a type variable), but we require that all variables introdruced by let statements must have monotype.

To adapt the `DeterminedType` predicate to polymorphism, we add the following rules to mirror our rules for type contexts:
...
```
Our rules will infer that `x` must be have function type `(a) => b`.
Since `x` is applied to itself, we infer a type equality `a = (a) => b`.
Now every instantiation of `foo` in which the type of `x` is concrete type (i.e., does not contain type variables) would result into a type conflict.
But if `foo` is never used, then our type checker will not report an error for it, since our rules for type conflicts take only concrete types into account but not type variables.

To detect such errors, we introduce a predicate `SmallerType : Type * Type` such that `SmallerType(sigma, tau)` holds if and only if `sigma` is structurally strictly smaller than `tau`:
```eqlog
