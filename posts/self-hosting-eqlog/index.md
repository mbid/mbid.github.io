...
This post is about my recent rewrite of Eqlog's type checker in Eqlog itself.

I finished prototyping a Hindley-Milner style type checker using Eqlog in a [previous series](../type-checking-with-eqlog-parsing).
...
One general topic that I do want to remark on is generating error messages, since I completely left this part out in the series on Hindley-Milner type checking.
To be easily actionable, error messages must be associated with a location in the input source code.
And to have access to locations after parsing too, we need to save locations along with AST nodes.
The approach Eqlog uses here now is to associate each AST node with a `Loc` element representing a location in source code:
```eql
type Loc;
...
In a traditional compiler, there is usually an ordering in which it encounters an error in the input source code.
