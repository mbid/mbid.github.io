...
*The main benefit of this language feature is to make it possible to compose small self-contained Datalog programs into larger programs, perhaps reminiscent of how classes in object oriented languages enable multiple instantiations of part of a program and its associated data.
So while this feature also enables the compiler to detect more programming errors, the main benefit is compositionality, not correctness.
The feature allows expressing locality to the Datalog compiler, which allows it to generate more efficient code.*

*Dependent Datalog is a further generalization of Cartmell's framework of generalized algebraic theories (GATs):
Where GATs have a notion of dependent set, dependent Datalog has a notion of dependent model for any given Datalog theory.*

This will be a rather conceptual post, and unfortunately I can't claim that I've fully worked through even the theoretical details let alone started to implement this.
But there's a good chance I'll be a bit busier soon, so I figured it might be better to write down some of those ideas in case I can get back to them later.

## A brief overview of Eqlog

In case you haven't followed the work on the Eqlog Datalog compiler or the similar [Egglog](https://github.com/egraphs-good/egglog) engine, let me try to briefly summarize.
Eqlog implements an extension of Datalog with support for (partial) functions and equality.
While I think that dependent types make sense for standard Datalog as well, dependent types interact non-trivally with equality, so I decided to write directly about dependent types in Eqlog.

Eqlog represents a function `f(a : A) -> B` by its graph, a predicate `graph_f : (a : A, b: B)` where `graph_f(x, y)` means that `f(x) = y`.
...
```
This is an example of the concrete input syntax accepted by the Eqlog compiler.
In standard Datalog syntax this would be written like so:
```prolog
y_0 = y_1 :- graph_f(x, y_0), graph_f(x, y_1).
```
(Of course, standard Datalog doesn't have equality, so the head would not be well-formed here.)

Predicate and function arguments are typed.
Types, functions and predicates are declared like so:
```eql
type A;
type B;
pred example_predicate(y: B, x: A);
func example_function(x_0: A, x_1: A) -> B;
```

Compiled Eqlog programs represent elements of types as integer IDs.
When the compiled Eqlog program infers an equality among elements `x` and `y` during evaluation, it will replace all occurrences of `y` in predicates and functions by `x`.

Another extension over standard Datalog is the ability to enforce that a function is defined on given arguments via the exclamation mark operator:
```eql
func f(x: A) -> B;
...
```
This feature makes Eqlog Turing complete.
...
Instead, the generated code is meant to be included into a host program written in Rust.
This generated code provides a data structure to represent the data declared in the Eqlog program:
There's a set of elements for each type in the Eqlog program, with each element represented by an integer ID, and sets of tuples to represent each predicate and function (via its graph) declared in the Eqlog program.
Finally, the generated code provides a `close` method that operates on this data structure which adds data according to the rules of the Eqlog program until all rules are satisfied.
...
This is a common subtask in compilers and static analysis.
A CFG is a representation of a function's execution flow as directed graph:
The nodes of the graph represent basic code blocks, and the edges represent possible direct execution flows between two basic blocks.
...
If we compile this Eqlog program, we get a module that provides a `CFG` data structure.
...
The problem is that we need need to instantiate separate CFGs for each function in our Eqlog program, but Eqlog does not provide a mechanism to dynamically instantiate fragments, for example the CFG program, dependent on other data.

We can work around this by parametrizing types, predicates and functions in our CFG program with elements of a new type that represents different instances of the original CFG program:
```eql
...
But while the total number of joins required is roughly the same with both appraoches, each join will be more costly with the second approach than with the first because the edge table is much larger.

## Model definitions and members
...
My hope is that adding dependent types to Eqlog will enable this.

The minimal version of dependent Eqlog consists of (1) a way to define named fragments of Eqlog code and (2) a way to access members definitions.
...
To enforce the surjectivity restrictions I alluded to earlier, Eqlog already decides element equality to some extent, so I don't think this would add significant additional complexity.
Note that Eqlog's element equality decision procedure takes only implicit functionality rules (i.e. `x = y => f(x) = f(y)`) into account, not other rules in the Eqlog program (this would make the type checker possibly non-terminating).
...
For `CFG` model example this would result in something like this Rust data structure:
```Rust
struct CFG(u32);
struct Node(u32);
...
Alternatively, we could compile the model body in isolation, which would give us a data structure representing just one instance of the model:
```Rust
struct Node(u32);
struct CFGMembers {
    edge: Set<(Node, Node)>,
...
But it also takes up more memory if the members of model instances are mostly empty, and it often makes it more costly to access members when evaluating rules of the ambient program.

Datalog engines often maintain indices on relations, i.e. multiple representations of the same data that are suitable for computing different types of joins efficiently.
So perhaps some implementations would simply use both representations to improve runtime performance at the cost of memory usage.

## Morphisms

Originally I intended to also write at length about *morphisms* between instances here, but decided to postpone a proper discussion to some other time, since I haven't worked out some relevant details.
The rough idea is that we add a built-in type of morphisms between two instances of the same model.
...
For example, if there's a morphisms `f : x -> y` and no other rules asserting things about the instance `y`, then tables in `y` can be represented simply as pointers to the tables of `x`.
...
The problem with this is that the program has to deal with a large number of AST nodes, and so the table representing `var_type` will be very large.
...
For example, expression (as opposed to statements) can usually not introduce new variables, so you can add a rule like this:
```
rule {
...
Every variable declaration increases the `var_type` table of the associated scope.
...
```
This means that whenever a new variable is introduced, we take a deep copy of the current scope and insert the new variable into it.
For a sequence of $n$ statements that each introduce a new variable, this would result in $\mathcal{O}(n^2)$ runtime and memory usage.
With morphisms, we could express that one scope is an extension of another.
If the compiler implements morphisms via shallow copies, then we'd cut the memory required for storgin scopes  down to $\mathcal{O}(n \cdot \mathrm{log}(n))$.
